...
public class FileStorageServiceImpl implements FileStorageService {
    ...
    public File storeFileDefault(Long userId) {
        try {
            User user = userService.findById(userId);
            Resource resource = new ClassPathResource("noavt.jpg");
            InputStream input = resource.getInputStream();
            byte[] array = Files.readAllBytes(Paths.get(resource.getURI()));

            File dbFile = new File("default", "image/jpeg", array);
            dbFile.setUser(user);
            return fileRepository.save(dbFile);
        } catch (IOException ex) {
            throw new FileStorageException("Could not store file default " + ". Please try again!", ex);
        }
    }
...
//            User user = userReponsitory.findById(userId).orElse(null);
    ...
    @Override
    public void deleteFileByIdProduct(Long idroduct) {
        Product product = productService.findById(idroduct);
        for (File file : fileRepository.findAll()) {
