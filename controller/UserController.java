...
public class UserController {
    ...
    @PutMapping(value = "/updateUserFile/{idUser}")
    public ResponseEntity<UserDTO> updateUserFIle(@ModelAttribute User user,
        ...
        if (file.isEmpty()) {
            ...
            data = userService.convert(user1);

            return new ResponseEntity<>(data, HttpStatus.OK);
        }
        else {
            /*fileStorageService.findByUserId(idUser);*/
            fileStorageService.deleteByIdUser(idUser);
            fileStorageService.storeFile(file,idUser);
