...
public class CommentController {
    ...
    public ResponseEntity<List<CommentDTO>> selectById(@PathVariable(value = "id") Long idProduct) {
        ...
        for (Comment data : dataList) {
            ...
            commentDTO.setUserDTO(commentService.convert(data.getUser()));
    ...
    /**
     * Method này cập nhât 1 row comment và file theo idComment
     *
     * @param : idComment
     * @api : http://localhost:8080/comment/update/...
     * @method : PUT
     */
    @PutMapping(value = "/update/{id}")
    public ResponseEntity<CommentDTO> updateComment(@PathVariable(value = "id") Long idComment,
                                                    @RequestParam(value = "commentContent") String commentContent,
                                                    @RequestParam(value = "files") MultipartFile[] files) {

        commentService.updateById(idComment, commentContent);
        ...
        for (MultipartFile file1 : files) {
            int i =1;
            if (file1.isEmpty()) {
                CommentDTO commentDTO = commentService.findById(idComment);
                return new ResponseEntity<>(commentDTO, HttpStatus.OK);
            }
            else {
                if(i==1){
                    fileStorageService.deleteByIdComment(idComment);
                }
                File file2 = fileStorageService.storeFileComment(file1, idComment);
                String fileDownloadUri = ServletUriComponentsBuilder.fromCurrentContextPath()
                        .path("file/downloadFile/")
                        .path(file2.getId())
                        .toUriString();
                urlImage.add(fileDownloadUri);
            }
            i++;
        }

        CommentDTO commentDTO = commentService.findById(idComment);
        commentDTO.setUrlFile(urlImage);
        return new ResponseEntity<>(commentDTO, HttpStatus.OK);
    }

    // Lấy ra Comment theo id comment truyền vào
    @GetMapping(value = "/selectByIdComment/{idComment}")
    public ResponseEntity<CommentDTO> finByIdComment(@PathVariable(value = "idComment") Long id) {
