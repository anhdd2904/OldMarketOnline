    ...
    public ResponseEntity<UploadFileResponse> registerUser(@ModelAttribute SignupRequest signUpRequest,
        ...
        if (userRepository.existsByUsername(signUpRequest.getUsername())) {
            return ResponseEntity
                    .badRequest()
                    .body(new UploadFileResponse("Tên tài khoản đã tồn tại!"));
        }

        if (userRepository.existsByEmail(signUpRequest.getEmail())) {
            return ResponseEntity
                    .badRequest()
                    .body(new UploadFileResponse("Email đã tồn tại!"));
        }
        ...
        if (strRoles == null) {
            Role userRole = roleRepository.findByName(ERole.ROLE_USER)
                    .orElseThrow(() -> new RuntimeException("Cấp độ người dùng không tồn tại!"));
            roles.add(userRole);
        } else {
        ...
        if (file.isEmpty()) {
            File dbFile = fileStorageService.storeFileDefault(idUser);
            String fileDownloadUri = ServletUriComponentsBuilder.fromCurrentContextPath()
                    .path("file/downloadFile/")
                    .path(dbFile.getId())
                    .toUriString();
            return ResponseEntity.ok(new UploadFileResponse(dbFile.getId(), dbFile.getFileName(), fileDownloadUri,
                    file.getContentType(), file.getSize(), "Đăng ký thành công với avatar mặc định"));
        }
        ...
        return ResponseEntity.ok(new UploadFileResponse(dbFile.getId(), dbFile.getFileName(), fileDownloadUri,
                file.getContentType(), file.getSize(), "Đăng ký thành công"));
    }
}
